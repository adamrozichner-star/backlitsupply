...
export function createStorage(): MockupStorage {
  ...
  // LocalStorage writes to public/mockups/ which is tracked in git.
  // After a batch run: git add public/mockups/ && git commit && git push
  // to deploy new mockups to Vercel. Verification tail catches any that
